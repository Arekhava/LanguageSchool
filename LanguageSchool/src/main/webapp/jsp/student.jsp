...
<meta charset="utf-8">
  ...
  <fmt:message key="local.cost" var="cost"/>
  <fmt:message key="local.payment_method" var="payment_method"/>
  <fmt:message key="local.search_subscriptions_by" var="search_subscriptions_by"/>
  ...
  <link rel="stylesheet" href="${pageContext.request.contextPath}/css/subscriptions/style.css" type="text/css" />
...
<div class="wrapper">
	...
	<div class="main">
		...
		<form action="${pageContext.request.contextPath}/controller" method="get">
			...
			<input type="number" name="subscriptionsId" required placeholder="№" min="1" max="9223372036854775807"/>
			<input type="submit" value="${search}"/>
		</form>
		<form action="${pageContext.request.contextPath}/controller" method="get">
			<label>${customer}:</label>
			<input type="hidden" name="command" value="find_user_subscriptions"/>
	...
	<table class="subscriptions_table">
		...
		<tr align="center" valign="center">
			...
			<td>
				<fmt:parseDate value="${subscription.dateTime}" pattern="yyyy-MM-dd'T'HH:mm" var="parsedDateTime" type="both" />
				<fmt:formatDate pattern="dd.MM.yyyy HH:mm" value="${parsedDateTime }" />
			</td>
			<td>${subscription.cost}$</td>
			<td><fmt:message key="local.payment_method.${subscription.paymentMethod}"/></td>
			<td><fmt:message key="local.subscription_status.${subscription.subscriptionStatus}" /></td>

			<td valign="center" align="center" >
				<c:if test="${sessionScope.role == 'ADMIN'}">
				<form class="process_subscription_form" action="${pageContext.request.contextPath}/controller" method="post">
					...
					<c:if test="${subscription.subscriptionStatus == 'COURSE_ADDED' || subscription.subscriptionStatus == 'ACCEPTED_TO_COURSE' || subscription.subscriptionStatus == 'ACTIVE_COURSE'}">
				...
				<form class="cancel_subscription_form" action="${pageContext.request.contextPath}/controller" method="post">
					...
					<input type="hidden" name="subscriptionId" value="${subscription.subscriptionId}"/>
					<input type="hidden" name="status" value="${subscription.subscriptionStatus}"/>
					<c:if test="${sessionScope.role == 'STUDENT' && subscription.subscriptionStatus == 'COURSE_ADDED'}">
					<input type="submit" value="${cancel}"/>
					</c:if>
					<c:if test="${sessionScope.role == 'STUDENT' && subscription.subscriptionStatus != 'COURSE_ADDED'}">
					<input type="submit" value="${cancel}" disabled/>
					</c:if>
					<c:if test="${sessionScope.role == 'ADMIN' && subscription.subscriptionStatus != 'NOT_ACCETT_TO_COURSE'}">
					<input type="submit" value="${cancel}"/>
					</c:if>
					<c:if test="${sessionScope.role == 'ADMIN' && subscription.subscriptionStatus == 'NOT_ACCETT_TO_COURSE'}">
					<input type="submit" value="${cancel}" disabled/>
			...
			<td valign="center" align="center" class="subscription_info">
				...
				<button id="details_id_button_${subscription.subscriptionId}" onclick="openDetails('courses_${subscription.subscriptionId}', 'details_id_button_${subscription.subscriptionId}', 'close_details_id_button_${subscription.subscriptionId}')">${detail}</button>
				<button id="close_details_id_button_${subscription.subscriptionId}" style="display:none;" onclick="closeDetails('courses_${subscription.subscriptionId}', 'close_details_id_button_${subscription.subscriptionId}', 'details_id_button_${subscription.subscriptionId}')">${close_details}</button>
			</td>
		</tr>
		<tr id="courses_${subscription.subscriptionId}" style="display:none;">
			<td colspan="10">
