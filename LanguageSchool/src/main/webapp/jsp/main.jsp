							...
							<p>
								<label>${login}<p>
										<input type="email" required placeholder="${email}"
											name="login" class="log" maxlength="45">
									</p></label>
							</p>
							<p>
								<label>${password}<p>
										<input type="password" required placeholder="${password}"
											name="password" class="log">
									</p></label>
							</p>
							<a
								href="${pageContext.request.contextPath}/jsp/forgot_password.jsp">${forgot_password}</a>
			...
			<c:if test="${sessionScope.languages != null}">
				<div class="languages">
					<c:forEach var="language" items="${sessionScope.languages}">
						...
						<div class="image">
							<a
								href="${pageContext.request.contextPath}/controller?command=show_courses_from_language&languageId=${language.languageId}"
								title="${language.languageName}"><img
								src="${pageContext.request.contextPath}/upload?imageName=${language.imageName}"
								alt="${language.languageName}"></a>
						</div>
						<div class="name">
							<a
								href="${pageContext.request.contextPath}/controller?command=show_courses_from_language&languageId=${language.languageId}"
								title="${language.languageName}"><fmt:message
									key="local.language.${language.languageName}" /></a>
						</div>
						</figure>
					</c:forEach>
					</figure>
			...
			<div class="courses">
				<c:forEach var="course" items="${requestScope.courses}">
					<figure class="course">
						<div class="image">
							<img
								src="${pageContext.request.contextPath}/upload?imageName=${course.imageName}"
						...
						<c:if test="${sessionScope.role == 'ADMIN'}">
							<button id="button_edit_${course.courseId}"
								onclick="openEditForm('form_edit_${course.courseId}', 'button_edit_${course.courseId}')">${edit}</button>
							<form class="form_edit" id="form_edit_${course.courseId}"
								action="${pageContext.request.contextPath}/controller"
								method="post">
								<span>${edit}</span> <input type="hidden" name="command"
									value="change_course_data" /> <input type="hidden"
									name="courseId" value="${course.courseId}" />
								<div>
									<label>${name}:<input type="text" name="courseName"
										value="${course.courseName}" /></label>
								</div>
								<div>
								...
								<input type="submit"
									onclick="closeEditForm('form_edit_${course.courseId}', 'button_edit_${course.courseId}')"
									value="${save}" />
							</form>
						</c:if>
					</figure>
