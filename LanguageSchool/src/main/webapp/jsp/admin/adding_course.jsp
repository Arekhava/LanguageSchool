...
<link rel="stylesheet"
	...
	type="text/css" />
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/adding_course/style.css"
	type="text/css" />
	...
	<div class="wrapper">
		...
		<div class="main">
			...
			<form class="add_course_form"
				...
				<div>
					...
					<select size="1" id="language" name="languageId" required>
						<c:forEach var="language" items="${sessionScope.languages}">
							<option value="${language.languageId}"><fmt:message
									key="local.language.${language.languageName}" /></option>
						</c:forEach>
					</select> <label>${language_name}:</label> <input type="text" name="name"
						required placeholder="${course_name}" maxlength="45" />
				</div>
				<div>
					<label>${price}:</label> <input type="text" name="price" required
						placeholder="${price}" pattern="\d{1,8}(\.\d{2})?" /> $
				</div>
				<div>
					<label>${upload_image}:</label> <input type="file" name="file"
						value="" height="150">
				</div>
				<div>
				...
				<div>
					<label>${course_id}:</label> <input type="text" name="courseId"
						required placeholder="${course_id}" maxlength="45" />
				</div>
				<div>
					<label>${upload_image}:</label> <input type="file" name="file"
						value="" height="150">
