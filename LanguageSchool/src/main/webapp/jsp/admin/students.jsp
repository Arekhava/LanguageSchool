...
<fmt:message key="local.title.students" var="title" />
	...
	<div class="wrapper">
		...
		<div class="main">
			...
			<form action="${pageContext.request.contextPath}/controller"
				method="get">
				<input type="hidden" name="command" value="find_user_by_login" /> <label>${email}:</label>
				<input type="email" required placeholder="${email}" name="login"
					maxlength="45" /> <input type="submit" value="${search}" />
			</form>
				...
				<table>
					...
					<c:forEach var="user" items="${requestScope.users}">
						<tr align="center" valign="center">
							...
							<td><c:if test="${user.status == 'BLOCKED'}">
									<form action="${pageContext.request.contextPath}/controller"
										method="post">
										<input type="hidden" name="command" value="unblock_user" /> <input
											type="hidden" name="userId" value="${user.userId}" /> <input
											type="submit" value="${unblock}" />
									</form>
									...
									<form action="${pageContext.request.contextPath}/controller"
										...
										<input type="hidden" name="command" value="block_user" /> <input
											type="hidden" name="userId" value="${user.userId}" /> <input
											type="submit" value="${block}" />
									</form>
								</c:if> <c:if test="${user.status == 'INACTIVE'}">
									<input type="submit" value="${block}" disabled />
								</c:if></td>
							<td>
								<form action="${pageContext.request.contextPath}/controller"
									method="post">
									<input type="hidden" name="command" value="send_message" /> <input
										type="hidden" name="email" value="${user.login}" /> <input
										type="text" name="message" required placeholder="${message}" />
