	...
	<div class="wrapper">
		...
		<div class="main">
			...
			<form action="${pageContext.request.contextPath}/controller"
				method="post" name="registration" class="edit clearfix">
				<input type="hidden" name="command" value="change_user_data" />
				<div>
					<label>${name}: <input type="text" name="userName" required
						placeholder="${name}" value="${user.name}"
						pattern="[a-zA-Zа-яА-Я-\s]{1,45}" />
					</label>
				</div>
				<div>
					<label>${phone}: <input type="tel" name="phone" required
						placeholder="+375XXXXXXXXX" value="${user.phone}"
						pattern="\+375\d{9}" />
					</label>
				</div>
				<div>
					<label>${email}: <input type="email" name="login" required
						placeholder="${email}" value="${user.login}" maxlength="45" />
					</label>
				</div>
				<div>
					<label>${enter_password}: <input type="password"
						name="password" required placeholder="${current_password}" />
					</label>
				</div>
			...
			<form action="${pageContext.request.contextPath}/controller"
				...
				<div>
					<label>${new_password}: <input type="password"
						name="newPassword" placeholder="${password_rules}"
						pattern="[A-Za-z\d]{5,15}" />
					</label>
				</div>
				<div>
					<label>${current_password}: <input type="password"
						name="password" required placeholder="${current_password}" />
					</label>
				</div>
				<input type="submit" value="${save}" />
