...
<fmt:message key="local.title.loked" var="title" />
...
<link rel="stylesheet"
	...
	type="text/css" />
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/liked/style.css"
	type="text/css" />
	...
	<div class="wrapper">
		...
		<c:if test="${not empty requestScope.subscription.courses}">
						...
						<td>
							<form action="${pageContext.request.contextPath}/controller"
								...
								<input type="hidden" name="courseId"
									value="${course.key.courseId}" /> <input type="submit"
									value="${save}" />
							</form>
						</td>
						...
						<td>
							<form action="${pageContext.request.contextPath}/controller"
								...
								<input type="hidden" name="command"
									value="remove_course_from_liked" /> <input type="hidden"
									name="courseId" value="${course.key.courseId}" /> <input
			...
			<div class="subscription">
				<div class="final_price">${in_total}:
					${requestScope.subscription.cost}$</div>
				<div class="payment">
					<form action="${pageContext.request.contextPath}/controller"
						method="post">
						<input type="hidden" name="command" value="checkout" /> <input
							type="hidden" name="cost"
							value="${requestScope.subscription.cost}" />
						<div>
							<label for="paymentMethod">${payment_method}:</label> <select
								size="1" id="paymentMethod" name="paymentMethod" required>
								<c:forEach var="paymentMethod"
									items="${requestScope.paymentMethodList}">
									<option value="${paymentMethod}"><fmt:message
											key="local.payment_method.${paymentMethod}" /></option>
								</c:forEach>
							</select>
						</div>
