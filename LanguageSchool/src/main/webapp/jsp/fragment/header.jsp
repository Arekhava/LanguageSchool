...
<fmt:message key="local.courses_in_subscription"
	var="courses_on_subscription" />
<header>
	<div class="header_top">
		<div>
			...
			<form action="${pageContext.request.contextPath}/controller"
				...
				<input type="text" required placeholder="${look_for}"
					name="courseName" /> <input type="submit" value="${search}" />
			</form>
			<nav class="menu">
				<ul class="menu1" id="menu1">
					<c:if test="${sessionScope.role != 'ADMIN'}">
						<li><a
							href="${pageContext.request.contextPath}/controller?command=go_to_liked_page">${liked}</a></li>
					</c:if>
					<li><a
						href="${pageContext.request.contextPath}/controller?command=go_to_main_page">${catalog}</a>
						<ul class="menu2">
							<c:forEach var="language" items="${sessionScope.languages}">
								<li><a
									href="${pageContext.request.contextPath}/controller?command=show_courses_from_language&languageId=${language.languageId}"><fmt:message
											key="local.language.${language.languageName}" /></a></li>
							</c:forEach>
							<c:if test="${sessionScope.role == 'ADMIN'}">
								<li><a
									href="${pageContext.request.contextPath}/jsp/admin/adding_course.jsp">${add_course}</a></li>
								<li><a
									href="${pageContext.request.contextPath}/controller?command=show_courses_in_subscription_subscription&page=1">${courses_in_subscription}</a></li>
							</c:if>
						</ul></li>
					<c:if test="${sessionScope.role == 'ADMIN'}">
						<li><a
					...
					<c:if test="${sessionScope.role == 'STUDENT'}">
						<li><a
							href="${pageContext.request.contextPath}/controller?command=go_to_profile_page">${profile}</a>
							<ul class="menu2">
								<li><a
									href="${pageContext.request.contextPath}/controller?command=go_to_profile_page">${personal_data}</a></li>
								<li><a
									href="${pageContext.request.contextPath}/controller?command=go_to_subscriptions_page">${my_subscriptions}</a></li>
								<li>
									<form action="${pageContext.request.contextPath}/controller"
										method="post">
										<input type="hidden" name="command" value="sign_out" /> <input
											type="submit" value="${sign_out}" />
									</form>
								</li>
							</ul></li>
					</c:if>
					<c:if test="${sessionScope.role == 'ADMIN'}">
						<li><a
							href="${pageContext.request.contextPath}/controller?command=go_to_students_page">${students}</a></li>
